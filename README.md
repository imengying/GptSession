...
Cloudflare Pages Function，经新加坡 ProxyIP 与 Cloudflare 原生 TCP Socket
建立经过证书校验的 TLS 连接访问 OpenAI。
...
- RT 通过新加坡线路向 OpenAI 换取 Access Token、ID Token 和新 Refresh Token，再生成可用的 Sub2API 或 CPA 凭证。
...
- ProxyIP 仅转发经过证书校验的 TLS 密文，无法读取其中的凭证内容。
...
- 仓库中的 `wrangler.jsonc` 已为 Pages Functions 开启 `nodejs_compat`。

默认使用 `sin.proxyip.cmliussss.net:443`。如需换成自己的新加坡 ProxyIP，
可在 Pages 项目中添加可选环境变量 `OPENAI_PROXY_HOSTS`，多个地址使用逗号分隔：

```text
203.0.113.10:443,proxy.example.com:443
```
...
src/server/openai-proxy.ts 新加坡 ProxyIP、TLS 和 HTTP 传输
...
tests/openai-proxy.test.ts    ProxyIP、HTTP framing 与安全边界测试
```
