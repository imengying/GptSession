...
JSON 的解析、转换、预览和打包均在本地浏览器内完成。RT 由浏览器直接连接
OpenAI 换取完整凭证；仅 AT 验证通过同源 Cloudflare Pages Function 转发。
...
- RT 由浏览器直接向 OpenAI 换取 Access Token、ID Token 和新 Refresh Token，再生成可用的 Sub2API 或 CPA 凭证。
- AT / RT 联网验证时，请使用 OpenAI 支持地区的网络节点。
...
- 仅 AT 验证使用固定连接 OpenAI 的 Pages Function，无数据库，页面运行时零第三方依赖。

## 隐私与安全

- JSON 文件不离开浏览器；RT 直接发送至 OpenAI，AT 通过本站 Pages Function 转发至 OpenAI。
- Pages Function 不记录、不缓存、不持久化 AT。
- 不写入 localStorage、sessionStorage、IndexedDB 或 Cookie。
- 页面 CSP 仅允许访问同源 AT 接口和固定 OpenAI OAuth 域名：`connect-src 'self' https://auth.openai.com`。
...
需要测试 AT 联网验证时，使用 Pages Functions 本地环境：
...
src/openai-refresh.ts      浏览器直连 RT、AT 验证及双 client_id 回退
src/core/redaction.ts      预览敏感字段脱敏
src/core/zip.ts            无依赖 ZIP 打包
functions/api/openai/      Cloudflare Pages AT 验证接口
